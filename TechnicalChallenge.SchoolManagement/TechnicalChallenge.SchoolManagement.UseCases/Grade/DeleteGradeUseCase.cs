    ...
    public class DeleteGradeUseCase<TInputEntity>
    {
        ...
        public async Task<ResponseDto<int>> ExecuteAsync(int id)
        {
            ...
            try
            {
                ...
                if (result == 0)
                {
                    responseDto.Errors.Add(new Dto.Error.ErrorDto { Message = $"No se pudo eliminar el grado con id {id}." });
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    Message = $"Ocurrió un error al tratar de eliminar el grado con id {id}"
                });
            }
            return responseDto;
        }
